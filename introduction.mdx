...
The `FAILED` state can be entered from `STARTING` or `STOPPING` if an unrecoverable error occurs. Once in `FAILED`, the component may transition to `STOPPING_PREP` for cleanup, or directly to `DESTROYED`.
