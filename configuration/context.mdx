...
| `sessionCookieHttpOnly` | `true` | Controls the `HttpOnly` flag on the session cookie. |
| `useHttpOnly` | `true` | Alias for `sessionCookieHttpOnly`. Set `HttpOnly` on the `JSESSIONID` cookie to prevent JavaScript access. |
